...
const FB_HEIGHT = 200; // 高さ (px)
const FB_WIDTH = null;
const FB_SPEED = 10;   // 1フレームあたりの移動速度(px)
...
function updateAndDraw() {
  if (ctx && canvas) {
    ...
    if (currentCharacterImage.complete && currentCharacterImage.naturalWidth !== 0) {
      ...
      ctx.drawImage(currentCharacterImage, 30, canvas.height - displayHeight, displayWidth, displayHeight);
    }
    ...
    if (showGoImage && !isGhostDefeated && goImage.complete && goImage.naturalWidth !== 0) {
      ...
      const x = canvas.width - displayWidth - 20;
...
function getRank(cpm, accuracy) {
  ...
  if (cpm >= 250 && accuracy >= 100) return "泰山府君";
  if (cpm >= 200 && accuracy >= 99) return "大陰陽師";
  if (cpm >= 180 && accuracy >= 98) return "天文博士";
  if (cpm >= 160 && accuracy >= 95) return "陰陽頭";
  if (cpm >= 140 && accuracy >= 90) return "陰陽師";
  if (cpm >= 120 && accuracy >= 85) return "陰陽生";
  if (cpm >= 100 && accuracy >= 75)  return "呪禁生";
  if (cpm >= 80 && accuracy >= 70)  return "見習い";
  if (cpm >= 60 && accuracy >= 65)  return "式神使いの卵";
  if (cpm >= 40 && accuracy >= 60)  return "式神";
  return "迷い人";
}
