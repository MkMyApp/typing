...
　const SPAWN_OFFSET = -200;
  const enemy = {
    img: img,
    x: (canvas ? canvas.width : 1024) + SPAWN_OFFSET,
...
function getRank(cpm, accuracy) {
  if (accuracy < 50) return "金魚";
  if (cpm >= 100 && accuracy >= 100) return "本マグロ";
  if (cpm >= 90 && accuracy >= 99) return "真鯛";
  if (cpm >= 80 && accuracy >= 98) return "鰤";
  if (cpm >= 70 && accuracy >= 95) return "鰹";
  if (cpm >= 60 && accuracy >= 90) return "鮭";
  if (cpm >= 50 && accuracy >= 85) return "鯵";
  if (cpm >= 40 && accuracy >= 80) return "秋刀魚";
  if (cpm >= 30 && accuracy >= 70) return "鰯";
  if (cpm >= 20 && accuracy >= 60) return "柳葉魚";
  if (cpm >= 10 && accuracy >= 50) return "メダカ";
  return "ミジンコ";
}
// ==================================================
//  各種イベントフック関数
