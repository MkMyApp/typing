...
const GAME_HEIGHT = parseInt(computedStyle.getPropertyValue('--game-height')) || 434;
...
function drawScore() {
  ...
  const margin = 15;
  const rectX = GAME_WIDTH - 200;
  const rectY = margin - 5;
  ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
  ctx.fillRect(rectX, rectY, 185, 35);

  ctx.fillStyle = '#ff0000';
  ctx.fillText(text, GAME_WIDTH - margin, margin);
  ctx.restore();
}
...
function getRankTitle(cpm, acc) {
  ...
  if (cpm >= 250 && acc >= 98) return "ドラゴンスレイヤー";
  if (cpm >= 190 && acc >= 95) return "竜騎士";
  if (cpm >= 140 && acc >= 90) return "竜牙士";
  if (cpm >= 100 && acc >= 85) return "竜狩り";
  if (cpm >= 70) return "討伐隊長";
  if (cpm >= 50) return "見習い猟兵";
  return "村の自警団";
}
