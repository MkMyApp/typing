...
const ENEMY_HEIGHT = 250; // 表示する高さ(px)
...
function spawnEnemy() {
  ...
  // ★ 画面上の配列をクリアして、新しい問題のフルーツ1個だけをセット
  activeEnemies = [enemy];
}
...
function getRank(cpm, accuracy) {
  if (accuracy < 50) return "腐った実"; 
  if (cpm >= 100 && accuracy >= 100) return "神々しい黄金の果実";
  if (cpm >= 90 && accuracy >= 99) return "幻の特選フルーツ";
  if (cpm >= 80 && accuracy >= 98) return "プレミアム完熟王";
  if (cpm >= 70 && accuracy >= 95) return "極上の甘熟フルーツ";
  if (cpm >= 60 && accuracy >= 90) return "食べ頃の旬フルーツ";
  if (cpm >= 50 && accuracy >= 85) return "みずみずしい果実";
  if (cpm >= 40 && accuracy >= 80) return "元気な若実";
  if (cpm >= 30 && accuracy >= 70) return "まだ青い果実";
  if (cpm >= 20 && accuracy >= 60) return "酸っぱい小粒";
  if (cpm >= 10 && accuracy >= 50) return "芽を出したばかり";
  return "腐った実";
}
