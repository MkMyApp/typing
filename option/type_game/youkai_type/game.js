...
const CENTER_IMAGE_HEIGHT = 400;  // 中央表示する画像の縦幅基準 (px)
...
function getRankTitle(cpm, acc) {
  if (acc < 70) return "怯える村人";
  if (cpm >= 70 && acc >= 98) return "伝説の大陰陽師";
  if (cpm >= 60 && acc >= 95) return "高名な修験者";
  if (cpm >= 50 && acc >= 90) return "熟練の退治屋";
  if (cpm >= 40 && acc >= 85) return "妖怪退治屋";
  if (cpm >= 30) return "村の用心棒";
  if (cpm >= 20) return "駆け出し法師";
  return "村の自警団";
}
