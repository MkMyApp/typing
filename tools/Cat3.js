...
// 表示幅（全角2, 半角1）を計算するヘルパー関数
function getVisualWidth(str) {
  let width = 0;
  for (let i = 0; i < str.length; i++) {
    const charCode = str.charCodeAt(i);
    // ASCII範囲内(0x7f以下)は1、それ以外は2
    width += (charCode <= 0x7f) ? 1 : 2;
  }
  return width;
}
...
function updateStatus(index) {
  ...
  // 文字数表示も見た目の幅（ch）に合わせる
  const chars = getVisualWidth(text);
  ...
  if (charCountLabels[index]) {
    charCountLabels[index].textContent = "Chars: " + chars + "ch";
  }
}
...
async function saveAs(index) {
  ...
  try {
    const suggestedName = fileNames[index] === "(none)" ? "untitled.html" : fileNames[index];
...
function countMax() {
  ...
  for (let i = 0; i < lines.length; i++) {
    const len = getVisualWidth(lines[i]);
    if (len > max) {
      max = len;
    }
  }
  ...
  if (maxLabel) {
    ...
    maxLabel.textContent = prefix + max + "ch";
  }
}